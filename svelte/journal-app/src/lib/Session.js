...
export class AuthenticationAttempts {
  ...
  getAttempts() {
    return parseInt(
      window.sessionStorage.getItem(this._attemptsKeyName) || 0,
      10
    );
  }
...
export async function HandleSession(maxAttempts) {
  ...
  if (!profile) {
    if (maxAttempts < 1 || attempts.increaseAttempts() < maxAttempts) {
      ...
      console.error(
        "No profile returned, but do not redirect because maximum attempts have passed"
      );
    }
  }
...
async function GetProfile() {
  ...
  if (window.location.hash) {
    const matchIdToken = window.location.hash.match(
